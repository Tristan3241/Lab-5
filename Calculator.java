...
public class Calculator {
	/**
	 * Takes an input command of two string tokens and simulates a calculator
	 ...
	 * a String array in the form of [command, number]. Valid commands are "negate"
	 * and "halve". Each command takes an int value as a parameter. e.g. "negate 5"
	 * passed in as an array ["negate", "5"] returns a result of -5 "halve 6" passed
	 * in as an array ["halve", "6"] returns a result of 3
	 *
	 ...
	 * you try and parse is not an int value). I.e. you do not need to explicitly
	 * throw this exception. (1) CalculatorException("Illegal Command") The input
	 * command is neither negate or halve. You must throw this exception explicitly.
	 * The line will look similar to "throw new CalculatorException(<<message>>);"
	 *
	 * @param tokens The input string to the program tokenized (split up). Should be
	 *               a String array of two elements: [command, number].
	 * @return The result of the calculator operation ("negate number" or "halve
	 *         number"). The halved number should be rounded down (i.e. the default
	 *         when doing int math).
	 ...
	 * calculator command. The token length should never be something other than 3.
	 * If it is, there is an issue in execute(). The input expects the three tokens
	 * formatted in a String array in the form of [num1, command, num2]. Valid
	 * commands are (numbers must be int values): "num1 + num2" => return num1 +
	 * num2 "num1 - num2" => return num1 - num2 "num1 / num2" => return num1 / num2
	 *
	 * If the input is invalid, exceptions will be thrown with the following
	 ...
	 * Command") The input command is neither +, -, or /. You must throw this
	 * exception explicitly. The line will look similar to "throw new
	 * CalculatorException(<<message>>);" (2) ArithmeticException The command is "/"
	 * and both numbers are int; the second number is 0. If you attempt to perform
	 * num1 / num2, this exception will automatically be performed. You should not
	 * need to explicitly throw this exception.
	 *
	 * @param tokens The input string to the program tokenized (split up). Should be
	 *               a String array of three elements: [num1, command, num2].
	 * @return The result of the calculator operation ("num1 + num2", "num1 - num2",
	 *         or "num1 / num2")
	 * @throws ArithmeticException   A division by zero has occured.
	 * @throws NumberFormatException Thrown if the first or third token is not
	 *                               convertible from String to int.
	 * @throws CalculatorException   Thrown if the second token is not a valid
	 *                               command ("+", "-", or "/")
	 */
	...
	/**
	 * Method to execute the expression encoded in a sequence of tokens. The user
	 * input is tokenized in parseAndExecute and the tokens are passed as an array
	 * to this method. For each number of tokens that the program receives, this
	 * method will behave in different ways. The methods calculateTwoTokens and
	 * calculateThreeTokens will be used as helper methods. They handle the cases
	 * where the number of tokens = 2 and = 3.
	 *
	 * That is, for each number of tokens, the program expects a certain format of
	 * input. When the input does not match its expectations, the program should
	 * throw an exception. Follow the below cases to determine when you should throw
	 * an exception. The cases outline all possibilities for inputs - all the ways
	 * an input can be formatted both correctly and incorrectly, and how to choose
	 * what to do as a result (operate normally or throw and exception).
	 *
	 ...
	 * @throws NumberFormatException Thrown if a numeric token is not convertible
	 *                               from String to int.
	 * @throws CalculatorException   Thrown if the command token is not a valid
	 *                               command ("quit", "negate", "halve" "+", "-",
	 *                               "/"). For the 3 tokens case, the command token
	 *                               is the second token (e.g. "1 + 3", + is the
	 *                               command). For the 2 tokens case, the command
	 *                               token is the first token (e.g. "halve 2", halve
	 *                               is the command.
	 */
	 ...
	 *
	 * Valid commands are: "quit" - the program should quit "negate" - a number is
	 * turned negative "halve" - a number divided by 2 "+" - two numbers should be
	 * added "-" - a number should be subtracted from another "/" - a number should
	...
	public static String parseAndExecute(String input) {
		...
		try {
			...
			result = "Calculator Exception, message is: " + e.getMessage();
			return result;
		}
